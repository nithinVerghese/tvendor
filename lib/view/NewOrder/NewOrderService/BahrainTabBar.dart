...
class _BahrainTabBarState extends State<BahrainTabBar>
  ...
  Widget build(BuildContext context) {
    ...
    return !isLoading ? Column(
      ...
      child: CircularProgressIndicator(color: UIColor.baseGradientLight,),
    );
  }
                              ...
                              child: Row(
                                children: [
                                  ...
                                  dataModel[index].services[i].serviceFee != null?Padding(
                                    ...
                                    child: Row(
                                      children: [
                                        dataModel[index].services[i].serviceNameEn.contains('Pickup Extra')? Container() :Text(
  ...
  void _hitApiServices(int index,String orderType) {
    ...
    dataModel.clear();
    setState(() {});
    services.fetchServices("${vendorCategoryModel[index].id}", 'Bahraini', orderType)
        .then((result) {
      if (result["status"] == true) {
  ...
  void hitRequestAPI(String order_id,String item_id,String reason,String need_delivery,String clearance_or_pickup,String requiredDocument) {
    print('item_id ==> $item_id');
    print('requiredDocument ${requiredDocument}');
    // print('******** ${vendorCategoryModel[index].id.toString()},${vendorCategoryModel[index].nameEn.toString()}, ');
    // dataModel.clear();
    // setState(() {});
    orderRequestController.fetchServices(order_id,item_id,reason,need_delivery,clearance_or_pickup,requiredDocument)
        .then((result) {
      if (result["status"] == true) {
        isServicePresent = "";
        // needsDelivery = "${result['needs_delivery']}";
